...
#Multiply by  
scaled_education_df_2011[,2:length(education_df_2011)] <- sapply(education_df_2011[,2:length(education_df_2011)], function(x) as.numeric(x))
...
tm_shape(sf_merged_df) +
  tm_polygons(col = "business")

#Plot all majors
tmap_mode("plot")
tm_shape(sf_merged_df) +
tm_layout(legend.show = TRUE, legend.position = c("right", "bottom"), title.size = 2) +
